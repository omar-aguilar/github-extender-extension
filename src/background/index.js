...
const repositoryConfigPromise = storageManager.getKey('repositoryConfig');
const globalConfigPromise = storageManager.getKey('globalConfig');

Promise.all([repositoryConfigPromise, globalConfigPromise])
  .then(([{ repositoryConfig }, { globalConfig }]) => {
    const addonManager = new AddonManager(repositoryConfig, globalConfig);
    const pageManager = new PageManager();

    storageManager.subscribe('globalConfig', addonManager._onGlobalConfigUpdate);
    storageManager.subscribe('repositoryConfig', addonManager._onRepositoryConfigUpdate);
    pageManager.subscribe(addonManager._onEventReceived);
  });
